#!/usr/bin/env bash
# Install the exact Kilix Temps source closure selected by this Kilix checkout.
set -euo pipefail
umask 077
...
KILIX_TEMPS_PREFIX="${KILIX_TEMPS_PREFIX:-$HOME/.local}"
...
KILIX_TEMPS_REPO="${KILIX_TEMPS_REPO:-https://github.com/itsmygithubacct/kilix-temps.git}"
KILIX_TEMPS_REF="${KILIX_TEMPS_REF:-4f4dc5051435fca76c0eb5d7aa8505427f62cf76}"
KILIX_TEMPS_PRESENTER_REPO="${KILIX_TEMPS_PRESENTER_REPO:-https://github.com/itsmygithubacct/kitty-frame-presenter.git}"
KILIX_TEMPS_PRESENTER_REF="${KILIX_TEMPS_PRESENTER_REF:-307d1d73c845e337cd051dab800e81277797cc8b}"
KILIX_TEMPS_SOFT_RASTER_PY_REPO="${KILIX_TEMPS_SOFT_RASTER_PY_REPO:-https://github.com/itsmygithubacct/soft-raster-py.git}"
KILIX_TEMPS_SOFT_RASTER_PY_REF="${KILIX_TEMPS_SOFT_RASTER_PY_REF:-42d24aac97e1817ea5848235ee60ad560012bfa7}"
KILIX_TEMPS_SOFT_RASTER_REPO="${KILIX_TEMPS_SOFT_RASTER_REPO:-https://github.com/itsmygithubacct/soft-raster.git}"
KILIX_TEMPS_SOFT_RASTER_REF="${KILIX_TEMPS_SOFT_RASTER_REF:-b42a8e4a4dc14e082f8971708dd9bca781f2699d}"

die() { printf 'kilix temps: %s\n' "$*" >&2; exit 1; }
log() { printf 'kilix temps: %s\n' "$*" >&2; }

usage() {
  cat <<'EOF'
usage: install-kilix-temps.sh [--force|--print-refs]
...
case "${1:-}" in
  ...
  --print-refs)
    printf '%s\n' \
      "kilix-temps=$KILIX_TEMPS_REF" \
      "kitty-frame-presenter=$KILIX_TEMPS_PRESENTER_REF" \
      "soft-raster-py=$KILIX_TEMPS_SOFT_RASTER_PY_REF" \
      "soft-raster=$KILIX_TEMPS_SOFT_RASTER_REF"
    exit 0 ;;
  -h|--help) usage; exit 0 ;;
  *) usage >&2; exit 2 ;;
esac
...
for value in \
    "$KILIX_TEMPS_REF" "$KILIX_TEMPS_PRESENTER_REF" \
    "$KILIX_TEMPS_SOFT_RASTER_PY_REF" "$KILIX_TEMPS_SOFT_RASTER_REF"; do
  [[ "$value" =~ ^[0-9a-fA-F]{40}$ ]] \
...
prefix="$(normalize_absolute "$KILIX_TEMPS_PREFIX")" \
  || die "KILIX_TEMPS_PREFIX must be a normalized absolute path"
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$state_dir/kilix-temps-install.lock"
  flock 9
fi
...
managed_sources="$source_home/.kilix-temps-sources"
...
temps_dir="$managed_sources/kilix-temps-$KILIX_TEMPS_REF"
presenter_dir="$managed_sources/kitty-frame-presenter-$KILIX_TEMPS_PRESENTER_REF"
soft_raster_py_dir="$managed_sources/soft-raster-py-$KILIX_TEMPS_SOFT_RASTER_PY_REF"
soft_raster_dir="$managed_sources/soft-raster-$KILIX_TEMPS_SOFT_RASTER_REF"
installed_bin="$prefix/bin/kilix-temps"
installed_library="$prefix/lib/kilix-temps/libsoft-raster.so"
stamp="$state_dir/kilix-temps-install.refs"
...
from kilix_temps.graphics import graphics_available
...
ensure_checkout "Kilix Temps" "$temps_dir" "$KILIX_TEMPS_REPO" "$KILIX_TEMPS_REF"
...
stamp_tmp="$(mktemp "$state_dir/.kilix-temps-refs.XXXXXX")" \
