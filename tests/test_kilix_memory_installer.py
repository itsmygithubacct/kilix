...
class KilixMemoryInstallerTests(unittest.TestCase):
    def make_repo(
        self,
        root: Path,
        name: str,
    ...
    def fixture_environment(self, root: Path) -> tuple[dict[str, str], Path]:
        build_script = textwrap.dedent(
            ...
            executable = prefix / "bin" / "kilix-memory"
            library = prefix / "lib" / "kilix-memory" / "libsoft-raster.so"
            executable.parent.mkdir(parents=True, exist_ok=True)
            library.parent.mkdir(parents=True, exist_ok=True)
            with zipfile.ZipFile(executable, "w") as archive:
                archive.writestr("kilix_memory/__init__.py", "")
                archive.writestr(
                    "kilix_memory/graphics.py",
        ...
        app, app_ref = self.make_repo(
            root,
            "app-origin",
            {"Makefile": makefile, "build_fixture.py": build_script},
        )
        ...
        environment = {
            **os.environ,
            ...
            "KILIX_STATE_DIRECTORY": str(
                root / "data" / "kilix" / "state"
            ),
            "KILIX_MEMORY_PREFIX": str(prefix),
            "KILIX_MEMORY_REPO": str(app),
            "KILIX_MEMORY_REF": app_ref,
            "KILIX_MEMORY_PRESENTER_REPO": str(presenter),
            "KILIX_MEMORY_PRESENTER_REF": presenter_ref,
            "KILIX_MEMORY_SOFT_RASTER_PY_REPO": str(binding),
            "KILIX_MEMORY_SOFT_RASTER_PY_REF": binding_ref,
            "KILIX_MEMORY_SOFT_RASTER_REPO": str(raster),
            "KILIX_MEMORY_SOFT_RASTER_REF": raster_ref,
        }
        return environment, prefix

    def test_fresh_install_clones_exact_closure_and_verifies_graphics(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            executable = prefix / "bin" / "kilix-memory"
            library = (
                prefix / "lib" / "kilix-memory" / "libsoft-raster.so"
            )
            stamp = (
                root / "data" / "kilix" / "state"
                / "kilix-memory-install.refs"
            )
            self.assertTrue(os.access(executable, os.X_OK))
            self.assertTrue(library.is_file())
            self.assertEqual(len(stamp.read_text().splitlines()), 4)
            self.assertIn("installed and verified", first.stderr)
            managed = root / "source" / ".kilix-memory-sources"
            for closure in stamp.read_text().splitlines():
                name, commit = closure.split("=", 1)
                self.assertTrue(
                    (managed / f"{name}-{commit}" / ".git").is_dir()
                )
    ...
    def test_existing_non_checkout_is_never_executed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            project = (
                root / "source" / ".kilix-memory-sources"
                / f"kilix-memory-{environment['KILIX_MEMORY_REF']}"
            )
            ...
            self.assertIn(
                "exists but is not a Git checkout", result.stderr
            )
