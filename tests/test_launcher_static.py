...
class KilixLauncherTests(unittest.TestCase):
    ...
    def test_update_supports_kilix_ref(self):
        ...
        self.assertIn('-c submodule.recurse=false', text)
        self.assertIn('checkout --detach "$_target"', text)
        self.assertIn('config --local submodule.recurse true', text)
    ...
    def test_clickable_chrome_status_items_are_wired(self):
        ...
        # The engine now keeps the top-bar widgets in their own package: the
        # settings keys and segment producers in providers, the action dispatch
        # in registry. Each assertion below names the file the thing actually
        # lives in rather than searching all of them, so dropping a widget from
        # one place cannot be masked by another still mentioning it.
        chrome = ROOT / "src" / "kitty" / "kilix_chrome"
        providers = (chrome / "providers.py").read_text()
        registry = (chrome / "registry.py").read_text()
        ...
        self.assertIn("KILIX_CHROME_CLOCK", providers)
        self.assertIn("KILIX_CHROME_VOLUME", providers)
        self.assertIn("KILIX_CHROME_TEMPERATURE", battery)
        self.assertIn("KILIX_CHROME_NETWORK", providers)
        self.assertIn("KILIX_CHROME_CALENDAR", providers)
        self.assertIn("KILIX_CHROME_CLOCK_FORMAT", providers)
        self.assertIn("clock_segment", providers)
        self.assertIn("clock_segments", providers)
        ...
        self.assertIn("volume_segment", providers)
        self.assertIn("thermal_segment", battery)
        self.assertIn("network_segment", providers)
        ...
        self.assertIn("kilix_temps_target", registry)
        self.assertIn("Kilix Temps", registry)
        ...
        self.assertIn("run_kitten_with_metadata('kilix_clock'", registry)
        ...
        self.assertIn("volume_target(", registry)
        self.assertIn("Volume Control", registry)
        battery = (ROOT / "src" / "kitty" / "kilix_battery.py").read_text()
        # The resolver moved into the providers module; kilix_battery keeps the
        # old name pointing at it, so anything importing it from there still
        # works. Assert the alias by name — dropping it is the regression this
        # line is here to catch.
        self.assertIn("kilix_volume_target = volume_target", battery)
        self.assertIn("which('kilix-volume')", providers)
        self.assertIn("which('pulsemixer') or which('alsamixer')", providers)
        self.assertIn("which('nmtui')", registry)
        self.assertIn("Network Connections", registry)
        self.assertTrue((ROOT / "src" / "kittens" / "kilix_clock" / "main.py").is_file())
        self.assertIn("toggle_battery_percent", registry)
